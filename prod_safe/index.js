...
const privateCommandMap = new Map();
const preprocessorMap = new Map();
...
if (!token) {
    ...
    if (cooldownDbType.toLowerCase() != 'sqlite' && cooldownDbType.toLowerCase() != 'mongodb') {
        ...
        cooldownDbType = cooldownDbType.toUpperCase();
    }
...
for (const cmd of commandMap) {
    ...
    if (preproc) {
        if (!preproc.saveCooldownInDb) {
            cooldownMap.set(cmd[0].name, new Map());
        }
        else {
...
for (const guild of privateCommandMap) {
    for (const cmd of guild[1]) {
        ...
        if (cast.preprocessor && !cast.preprocessor.saveCooldownInDb) {
            cooldownMap.set(`${guild[0]}_${cast.slashCommand.name}`, new Map());
        }
        else {
...
await init(cooldownDbType.toUpperCase(), initCooldownMap, cooldownDbUri);
...
const elapsedTime = (endTimer - startTimer) / 1000;
                ...
                if (imp.slashCommand != undefined && imp.execute != undefined) {
                    ...
                    if (commandMap.has(imp.slashCommand.name) && !imp.guildRegisterId) {
                        ...
                        if (imp.guildRegisterId) {
                            if (!privateCommandMap.has(imp.guildRegisterId))
                                privateCommandMap.set(imp.guildRegisterId, []);
...
async function regCommands(guildId, commands) {
    let cmds = commands !== null && commands !== void 0 ? commands : undefined;
    if (!cmds) {
        cmds = [];
        for (const cmd of commandMap) {
            cmds.push(cmd[0]);
    ...
    let cmdJson = cmds.map(command => command.toJSON());
    if (guildId != undefined) {
        await client.application.fetch();
        let route = Routes.applicationGuildCommands(client.application.id, guildId);
        ...
        let route = Routes.applicationCommands(client.application.id);
        await rest.put(route, { body: cmdJson })
            .catch(reason => {
