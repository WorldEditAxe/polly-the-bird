...
export async function execute(i) {
    ...
    const info = await fetchDonos(user.id), cumulativeTotal = info.giveaways + info.heists + info.events + info.special;
    return await i.reply({ embeds: [
            new MessageEmbed()
                ...
                .addFields({ name: 'Giveaway Donations', value: stringize(info.giveaways), inline: true }, { name: 'Heist Donations', value: stringize(info.heists), inline: true }, { name: 'Event Donations', value: stringize(info.events), inline: true }, { name: 'Special Donations', value: stringize(info.special), inline: true }, { name: 'Money Donations', value: stringize(info.money, true), inline: true }, { name: 'Total (excluding money)', value: stringize(cumulativeTotal), inline: true })
        ] });
}
