...
export async function fetchDonos(userId) {
    // exists check
    let fetched = await db.findOne({ user_id: userId });
    if (!fetched) {
...
export async function addTo(userId, type, amount, moderator, sub) {
    ...
    if (type == 'MONEY')
        eventBindings.emit('totalUpdateMoney', fetched.money + amount, userId);
    else
        eventBindings.emit('totalUpdate', fetched.events + fetched.giveaways + fetched.heists + fetched.special + (sub ? amount + (amount * -2) : amount), userId);
}
export async function massInsert(arr) {
    await db.insertMany(arr);
}
export async function takeFrom(userId, type, amount, moderator) {
    await addTo(userId, type, amount - (amount * 2), moderator, true);
}
export async function setDono(userId, type, amount, moderator) {
    const fetched = await fetchDonos(userId);
    await db.updateOne({ user_id: userId }, { $set: { [type.toLowerCase()]: amount } });
    eventBindings.emit('set', userId, type, amount, moderator);
    if (type == 'MONEY')
        eventBindings.emit('totalUpdateMoney', amount, userId);
    else {
        let total = 0, loweredType = type.toLowerCase();
        for (const ent of Object.entries(fetched)) {
            if (ent[0] != loweredType && ent[0] != "user_id" && ent[0] != 'money' && ent[0] != '_id')
                total += ent[1];
        }
        total += amount;
...
export async function deleteUser(userId, moderator) {
...
export function prettify(num, money) { return money ? format_money.format(num) : instance.format(num); }
export function stringize(num, money) { return `\`${money ? '' : `${currency_symbol} `}${prettify(num, money)}\``; }
//# sourceMappingURL=donoDb.js.map