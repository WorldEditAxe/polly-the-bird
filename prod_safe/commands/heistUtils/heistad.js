...
function shortenAmount(amt) {
    const bil = amt / 1000000000, mil = amt / 1000000;
    if (bil > 1 && Math.floor(bil) == bil)
        return `${bil} billion`;
    else if (bil > 1)
        return `${bil.toFixed(2)} billion`;
    else
        return `${Math.floor(mil) == mil ? mil : mil.toFixed(2)} million`;
}
//# sourceMappingURL=heistad.js.map