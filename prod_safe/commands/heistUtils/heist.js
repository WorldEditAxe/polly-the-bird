...
let joinedUsers, startTime, heistMode = false;
...
export async function execute(i) {
    if (!i.guild)
        return await i.reply({ content: "I don't know what made you think this but you need to run this in a server??", ephemeral: true });
    if (!i.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES))
        return await i.reply({ content: "You are missing the permission 'Manage Messages' required to run this command!", ephemeral: true });
    const mode = i.options.getSubcommand(true) == 'enable' ? true : false, banFreeloaders = mode == false ? i.options.getBoolean('dump_freeloaders') : false;
    setHeistMode(mode);
    if (!banFreeloaders) {
        if (mode == true) {
            joinedUsers = [];
        ...
        if (!joinedUsers)
            return await i.reply({ content: "Heist mode is off lmao, I can't ban freeloaders", ephemeral: true });
...
export async function staticBlock() {
    global.bot.djsClient.on('guildMemberAdd', m => {
        if (Date.now() - m.user.createdAt.getTime() <= 2678000000 || lockDown)
            return;
        if (heistMode && !joinedUsers[m.user.id])
            joinedUsers.push(m.user);
