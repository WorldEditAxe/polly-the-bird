...
function getAttachmentString(attachments, short) {
    let ret = "", i = 0;
    if (!short) {
...
export async function execute(i) {
    if (!i.guild)
        return await i.reply({ content: 'imagine not using this in a server smfh', ephemeral: true });
    if (!i.guild.me.permissionsIn(i.channel).has(Permissions.FLAGS.VIEW_CHANNEL))
        return await i.reply({ content: "I cannot snipe in a channel where I do not have the permissions to view sent messages in.", ephemeral: true });
    if (!i.member.roles.cache.has('799022090791419954') && !i.member.roles.cache.has('929898376001122325') && !i.member.roles.cache.has('785676961904852992'))
        return await i.reply({ content: "You need to be a 25 million donor in order to use this command!", ephemeral: true });
    ...
    if (list) {
        ...
        if (!msg || msg.length < 1)
            return await i.reply({ content: "there aren't any sniped messages <:Kek:790750951882358835> tf u lookin for", ephemeral: true });
        ...
        for (const message of msg) {
            if (message.type == 'DELETED') {
                ...
                if (message.attachments.length > 0)
                    str += `**Attachments: ** ${getAttachmentString(message.attachments, true)}\n`;
                ...
                // TODO: work on esnipe
                let str = `**Author : **${message.author.tag}\n`;
                if (message.attachments.old.length > 0)
                    str += `**Old Attachments: ** ${getAttachmentString(message.attachments.old, true)}\n`;
                if (message.attachments.new.length > 0)
                    str += `**New Attachments: ** ${getAttachmentString(message.attachments.new, true)}\n`;
        ...
        if (m.type == 'DELETED') {
            ...
            const emb = new MessageEmbed()
                ...
                .setAuthor({ name: m.author.username, iconURL: m.author.avatarURL() })
                .addFields({ name: "Old Content", value: m.oldContent ? m.oldContent.length > 1024 ? m.oldContent.slice(0, 1021) + '...' : m.oldContent : "<nothing, maybe an embed or some attachments?>" }, { name: "New Content", value: m.newContent ? m.newContent.length > 1024 ? m.newContent.slice(0, 1021) + '...' : m.newContent : "<nothing, maybe an embed or some attachments?>" })
...
export async function staticBlock() {
    ...
    client.on('messageDelete', async (m) => {
        if (!m.guild || m.author.bot)
            return;
        if (!snipe.has(m.channelId))
            snipe.set(m.channelId, new Array());
        const attachments = [];
    ...
    client.on('messageUpdate', (before, after) => {
        if (!after.guild || after.author.bot)
            return;
        if (!snipe.has(after.channelId))
            snipe.set(before.channelId, new Array());
...
function appendAndClamp(array, element, limit) {
