...
export async function execute(i) {
    ...
    if (type == 'create') {
        if (reason.length > 50)
            return await i.reply({ content: `Your timer description is too long. Please limit the length of your description to be \`50\` characters or shorter.` });
        if (time.length > 15)
            return await i.reply({ content: "Your timer's length is excessively long.", ephemeral: true });
        let parsedTime;
        ...
        if (parsedTime - instant > 157680000)
            return await i.reply({ content: "Your timer's length is excessively long. The max length of a timer is 5 years.", ephemeral: true });
        await i.reply({
            embeds: [
                new MessageEmbed()
                    .setTitle(reason)
        ...
        if (!i.memberPermissions.has(Permissions.FLAGS.MANAGE_MESSAGES))
            return await i.reply({ content: "You do not have the permission 'Manage Messages' required to run this command!", ephemeral: true });
        await i.deferReply();
        let msg;
        ...
        const chan = await i.client.channels.fetch(timerId.channelId).catch();
        if (chan.guildId != i.guildId || !chan || !(chan.permissionsFor(i.guild.me).has(Permissions.FLAGS.SEND_MESSAGES)
        ...
        msg = await chan.messages.fetch(timerId.messageId).catch();
        if (!msg) {
