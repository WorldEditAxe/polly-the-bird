...
client.on('presenceUpdate', async (oldStatus, newStatus) => {
    ...
    if (mbr.user.bot || newStatus.guild.id != '784491141022220309')
        return;
    ...
    if (mbr.roles.cache.has(PREMIUM_ROLE) && !hasContentInStatus)
        await mbr.roles.remove(PREMIUM_ROLE).catch(() => { });
    else if (hasContentInStatus)
        await mbr.roles.add(PREMIUM_ROLE).catch(() => { });
});
export {};
