...
const broadcastChan = await client.channels.fetch(BROADCAST_ID);
const flaggedMessages = new Map();
const delId = "polly-delete-automod-alert-tag";
client.on('messageCreate', async (m) => {
    if (m.guildId != GUILD_ID || !m.content || m.author.bot || m.author.system)
        return;
    if (m.member.roles.cache.some(r => WHITELISTED_ROLE_IDS.includes(r.id)))
        return;
    if (await isStringOffensive(m.content))
        await handleFlagged(m);
});
client.on('interactionCreate', async (i) => {
    if (!i.isButton())
        return;
    if (i.customId != delId || i.channelId != BROADCAST_ID || i.guildId != GUILD_ID)
        return;
    await i.message.delete();
    await i.deferUpdate();
});
async function handleFlagged(m) {
...
//# sourceMappingURL=content_filter.js.map