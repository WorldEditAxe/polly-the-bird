...
const message = await (await client.channels.fetch('939984413733847130')).messages.fetch('940018823791280178');
const collector = message.createReactionCollector({ dispose: true });
collector.on('collect', async (e, usr) => {
    if (!e.message.guild)
        return;
    if (e.message.id != '940018823791280178')
        return;
    // no partner
    ...
    if (e.emoji.id == '793151158281175064' && (member.roles.cache.has('784549344225263646') || member.roles.cache.has('785201531656994837'))) {
        ...
        try {
            if (e.emoji.id == '936156459442536458')
                await member.roles.remove('784549344225263646');
            else
                await member.roles.remove('785201531656994837');
        }
        catch (_a) {
            // do nothing
            return;
...
//# sourceMappingURL=noPartner.js.map