...
export async function execute(i) {
    ...
    if (!ALLOWED_ROLES.some(allowed => i.member.roles.cache.has(allowed)))
        return await i.reply({ content: "You are not authorized to run this command.", ephemeral: true });
    ...
    switch (i.options.getSubcommand(true)) {
        default:
            ...
            break;
        case 'remove':
            if (dbEntry.getGrants().length <= 0)
                return await i.reply({ embeds: [getErrorEmbed("That user does not have any removable grants!")], ephemeral: true });
            break;
    }
}
0;
