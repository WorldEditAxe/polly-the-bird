...
export async function execute(i) {
    ...
    if (!i.member.roles.cache.some(v => authRoles.includes(v.id))) {
        await i.reply({ content: "You are not authorized to run this command!", ephemeral: true });
        return;
    }
    // options
    let type = i.options.getString("type"), description = i.options.getString("description").replace(/{ret}/gi, "\n"), prize = i.options.getString("prize").replace(/{ret}/gi, "\n"), time = i.options.getString("time"), donor = i.options.getUser("donor");
    // validity checks
    ...
    await i.deferReply();
    if (donor)
        donor = await i.guild.members.fetch(donor.id);
    ...
    emb.setColor('#fcba03')
        ...
        .setFooter(`ID: ${i.member.id}`);
    if (donor)
        emb.addField('Donor', `<@${donor.id}> (${donor.user.tag}, id: ${donor.id})\nThank the donor in <#870193314413019216>`, true);
    await channel.send({
        embeds: [emb],
...
//# sourceMappingURL=post_event.js.map