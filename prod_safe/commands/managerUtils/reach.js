...
export async function execute(i) {
    ...
    let roles;
    try {
        roles = await parseRoleString(i.options.getString("roles"), i.guild);
    }
    catch (err) {
    ...
    if (channel instanceof TextChannel)
        channel = await i.guild.channels.fetch(channel.id);
    for (const role of roles) {
        if (role == 'here') {
            ...
            for (const member of members.values()) {
                ...
                if (!member.user.bot && channel.permissionsFor(member).has(Permissions.FLAGS.VIEW_CHANNEL)) {
            ...
            const insEntry = { roleMembers: role.members.values(), reachableMembers: [], id: role.id, name: role.name };
            const rmArr = [];
            ...
            role.members.forEach(m => {
                // perm check
                if (!m.user.bot && channel.permissionsFor(m).has(Permissions.FLAGS.VIEW_CHANNEL)) {
    ...
    for (const ent of stats) {
        for (const m of ent[1].reachableMembers) {
            if (!cumulativeMembers.reachable.includes(m.id))
                cumulativeMembers.reachable.push(m.id);
        }
        for (const m of ent[1].roleMembers) {
            if (!cumulativeMembers.raw.includes(m.id))
                cumulativeMembers.raw.push(m.id);
        }
    }
    // construct embed
...
async function parseRoleString(rawArgs, guild) {
    const argPassedRoles = [], guildRoles = await guild.roles.fetch();
    const args = rawArgs.split(/, |,/g);
    const returnRoles = [];
    if (args.length > 10)
        throw new Error("Too many roles to look through! Please limit the amount of roles to `10` or less.");
    for (const role of args) {
        if (isNaN(parseInt(role))) {
            // treat as role name
            const matchTolerance = 0.85, loweredRole = role.toLowerCase();
            let found = false;
            if (loweredRole == "here" && !returnRoles.includes("here")) {
                returnRoles.push("here");
            }
                ...
                for (const guildRole of guildRoles.values()) {
                    if (similarity(loweredRole, guildRole.name.toLowerCase()) >= matchTolerance) {
                        if (!returnRoles.includes(guildRole))
                            returnRoles.push(guildRole);
                        found = true;
                        break;
                    }
                }
                if (!found)
                    throw new Error(`There is no role under the name of "${role}"!`);
            }
        }
        else {
            // treat as role ID
            if (!guildRoles.has(role))
                throw new Error(`Role ${role} is not a valid role ID!`);
            const rle = guildRoles.get(role);
            if (!returnRoles.includes(rle))
                returnRoles.push(rle);
        }
    }
    return returnRoles;
}
