 ...
 */
export async function preprocess(i, commandName, preprocessor, privateGuildId) {
    // guild check
    if (!i.guild && preprocessor.serverOnly)
        return "This command is guild/server only!";
    ...
    if (i.guild && preprocessor.requiredPermissions && !i.memberPermissions.has(Permissions.FLAGS.ADMINISTRATOR)) {
        ...
        for (const perm of preprocessor.requiredPermissions) {
            if (!i.memberPermissions.has(perm))
                missingPerms.push(new Permissions(perm).toArray()[0]);
        }
    ...
    if (i.guild && preprocessor.botPermissions && !i.guild.me.permissions.has(Permissions.FLAGS.ADMINISTRATOR)) {
        ...
        for (const perm of preprocessor.botPermissions) {
            if (!i.guild.me.permissions.has(perm))
                missingPerms.push(new Permissions(perm).toArray()[0]);
        }
        if (missingPerms.length > 0) {
            let missingPermString = "", index = 0;
    ...
    if (preprocessor.saveCooldownInDb) {
        if (preprocessor.cooldown) {
            const cooldown = await getCooldown(privateGuildId ? `${privateGuildId}_${commandName}` : commandName, i.user.id), time = Math.floor(Date.now() / 1000);
            if (cooldown > time) {
        ...
        const arr = cooldownMap.get(privateGuildId ? `${privateGuildId}_${commandName}` : commandName), val = arr.get(i.user.id), time = Math.floor(Date.now() / 1000);
        if (!val) {
