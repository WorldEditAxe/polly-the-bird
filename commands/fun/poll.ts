...
export async function execute(i: CommandInteraction) {
    ...
    if (!i.guild.me.permissionsIn(i.channel as GuildChannel).has(Permissions.FLAGS.SEND_MESSAGES)) return await i.reply({ content: "I do not have the permissions to send messages in this channel!", ephemeral: true })
