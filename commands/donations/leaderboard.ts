...
export async function execute(interaction: CommandInteraction) {
    ...
    let leaderboard: typeof schema[], str = `**Showing top #10 for type ${type.toLowerCase()}.**\n`

    if (type != 'TOTAL') {
        leaderboard = (await db.find({ [type.toLowerCase()]: { $gt: 0 } }).sort({ [type.toLowerCase()]: -1 }).limit(10).toArray() as any) as typeof schema[]
        ...
        leaderboard = (await db.aggregate([ {$project: {giveaways: 1, heists: 1, events: 1, special: 1, user_id: 1, orderBySumValue: { $add: [ "$giveaways", "$heists", "$events", "$special" ] } } }, { $sort: { orderBySumValue: -1 }}, { $limit: 10 } ]).toArray() as any) as typeof schema[]
