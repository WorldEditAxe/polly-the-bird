...
export function checkUserExpiry(dbEnt: schema$userEntry) {
...
async function loop() {
    while (true) {
        ...
        if (shouldSweep) {
            ...
            await getAll()
                .forEach((ent: WithId<schema$userEntry>) => {
