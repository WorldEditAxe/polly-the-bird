...
function convertStringToEmotes(text: string): string {
...
export async function execute(i: CommandInteraction) {
    if (!WHITELISTED_ROLES.some(role => (i.member as GuildMember).roles.cache.has(role))) return await i.reply({ content: "You do not have permission to use this command!", ephemeral: true })
    if (i.options.getString('text').length * 35 * 2 + 33 > 2000) return await i.reply({ content: "That string is too long for me to send due to Discord limitations. Sorry!" })
