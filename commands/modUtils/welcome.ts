...
const chan = await client.channels.fetch(welcomeChannel) as TextChannel, goodbyeChan = await client.channels.fetch(goodbyeChannel) as TextChannel, guild = chan.guild
...
cachedWelcomeEmbed
    ...
    .addFields(
        ...
        { name: 'Reaction Roles', value: "We have plenty of reaction roles you can choose from - please check them out. They are at <#939984413733847130>! Please note you are required to verify in order to access these channels." },
    )
    .setColor('#46d0ee')
...
client.on('interactionCreate', async e => {
    ...
    if (e.customId.startsWith('ar-')) await handleRoleInteraction(e)
})

async function handleRoleInteraction(event: ButtonInteraction) {
    try {
        ...
        try {
            ...
            switch(event.customId as roleButtonTypings) {
                default:
